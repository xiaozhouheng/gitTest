...
<script type="text/javascript">
    ...
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
        ...
        table.render({
            elem: '#ordermanage',
            url:'<%=path%>/AArrivalPlan/getAllAArrivalPlan.do',
            page: true,
            method:'get',
            height:600,
            limit:50,
            ...
            cols: [[
                ...
                {field:'param1',  title: '业务编码',align :'center'},
                {field:'param2',  title: '销售员代码',align :'center'},
                {field:'importPort',  title: '进出口岸',align :'center'},
                {field:'estimatedArrivalTime',  title: '预计到港时间',align :'center',templet :function (row){
                    return createTime(row.estimatedArrivalTime);
                }},
                ...
                {field:'fenWaybillNumber', title: '分运单号',align :'center'},
                {field:'supervisionType',  title: '监管方式',align :'center',templet :function (row,value){
                    return value ===1?"方式四":"方式三"}},
                {field:'transactionType',  title: '成交方式',align :'center',templet :function (row,value){
                    return value ===1?"方式二":"方式一"}},
                {field:'customsOrderNumber',  title: '报关单号',align :'center' },
                {field:'uniformNumbers',  title: '统一编号',align :'center'},
                {field:'customsCode',  title: '企业海关编码',align :'center'},
        ...
        ,done: function(res, page, count){
            ...
            $('.layui-table-body').find("table" ).find("tbody").children("tr").on('dblclick',function(){
                ...
                location.href = "<%=path%>/AArrivalPlan/enditAArrivalPlan.do?id="+ids;
            });
   	 ...
   	 var $ = layui.$, active = {
			    reload: function(){
	              ...
	              var fenWaybillNumber = $("#fenWaybillNumber").val();
			      //执行重载
    ...
    function removeData(){
    	...
    	$.ajax({
    	      url:"<%=path%>/AArrivalPlan/deleteAArrivalPlan.do?ids="+id,
    	      async: false,
    	      type:"POST",
    	      dataType: "text",
    ...
    function uploadFile(data){
    	...
    	var id = data.id;
    	var btn = "<a class='layui-btn-blue layui-btn-xs' lay-event='update' onclick='getUploadFile(\""+invoice+"\",\""+id+"\")' >上传附件</a>"
    	return btn;   
      }
    function getUploadFile(invoice,id){
    	var invoice = invoice;
    	var id = id;
        var urls = "<%=path%>/page/pageContract.do?path=views/aArrivalPlan/uploadFile";
		...
		<div class="container">
			<ul class="myorder-dropdown ">
				<li role="presentation"><a
					href="<%=path%>/page/pageJump.do?path=views/aArrivalPlan/aArrivalPlan">到货计划表</a>
				</li>
					...
					<div class="order-toolbar clearfix  col-lg-12">
						...
						<div class="pull-right xoms-keyword-div received-search-container">
							<form id="queryForm" class="form-inline xoms-search-form myorder-simple-search"
								...
								<label>分运单号:</label><input type="text" id ="fenWaybillNumber" class="form-control" name="fenWaybillNumber" value="">
