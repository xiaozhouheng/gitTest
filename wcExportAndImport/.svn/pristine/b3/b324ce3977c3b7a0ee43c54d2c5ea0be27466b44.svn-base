...
<title><spring:message code="toOrder.Draft" /></title>
<link rel="stylesheet"
...
<script type="text/javascript">
    ...
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
        ...
        table.render({
            elem: '#ordermanage',
            url:'<%=path%>/orderManage/findAll.do?notEnd=0',
            page: true,
            method:'post',
            height:600,
            limit:50,
   	 ...
   	 var $ = layui.$, active = {
			    ...
			    deleteOrder:function(){//支付
			    	 var obje = $("#queryForm").serializeJson();
			    	 var checkStatus = table.checkStatus('queryOrders');
				      ...
				      if(length!=0){
					       layer.confirm("确定支付这<font color='red'>"+length+"</font>条订单？", function(index){ 
									...
									$.post('<%=path%>/orderManage/deleteOrder.do',{ids:ids},
											function(rr){
										var result=eval('('+rr+')');
										if(result.result!=1){
			    ...
			    getPayOne:function(){
			    	 ...
			    	 var checkStatus = table.checkStatus('queryOrders');
				      ...
				      var length=i_ids.length;
				      if(length!=0){
					       layer.confirm("确定核算运费这<font color='red'>"+length+"</font>条订单？", function(index){ 
									var ids="";
									for(var i=0;i<length;i++){
										if(i==0){
											ids += i_ids[i];
									...
									$.post('<%=path%>/orderManage/getmailNOMoey.do',{ids:ids},
											function(rr){
										var result=eval('('+rr+')');
										if(result.result!=0){
											layer.msg(result.result,{icon:2});
			    ...
			    getCheckData: function(){ //获取选中数据导出
				      var checkStatus = table.checkStatus('queryOrders');
				      var data = checkStatus.data;
				      var length=i_ids.length;
...
-->
	...
	<div class="xoms-container" role="main">
		<div
						...
						<div class="pull-left xoms-btn-container operate-btn-container">
							<a class="btn layui-btn layui-btn-normal layui-btn-sm"
								...
								class="glyphicon glyphicon-arrow-down export"></span> <spring:message
									code="toOrder.ExportExcel" /></a> <a
								class="btn layui-btn layui-btn-normal layui-btn-sm"
								data-type="getPayOne"><span class="glyphicon glyphicon-ok"></span>
							<spring:message code="toOrder.confirm" /></a>
								<a class="btn layui-btn layui-btn-normal layui-btn-sm " data-type="deleteOrder">
									<span class="glyphicon glyphicon-ok"></span>
							<spring:message code="toOrder.deleteOrder" /></a>
							<!--    <a class="btn layui-btn layui-btn-normal layui-btn-sm" data-type="getPay"><span class="glyphicon glyphicon-jpy"></span>支付</a> -->
							<%--   <button class="btn btn-primary btn-sm min-screen-hidden tick"><span class="glyphicon glyphicon-ok tick"></span> 勾选未打印</button>
                 <button class="btn btn-primary btn-sm min-screen-hidden disabled print"><span class="glyphicon glyphicon-print print"></span><spring:message code="toOrder.PrintLabel"/></button>
                 <div class="btn-group">
                     ...
                     <div class="form-group other-value-group">
                         ...
                         <button type="button" class="btn btn-default expand"><span class="glyphicon glyphicon-chevron-up expand"></span></button>
								...
								<%-- <input type="text" name="orderOutCustom.orderNo" id="orderNo" class="form-control" placeholder="<spring:message code="errOrder.please"/>">   --%>
			...
			<table id="ordermanage" lay-filter="queryOrders"
