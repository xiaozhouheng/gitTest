...
<html>
<head>
<title><spring:message code="toOrder.Received" /></title>
<link rel="stylesheet"
...
<script type="text/javascript">
    ...
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
        ...
        table.render({
            elem: '#ordermanage',
            url:'<%=path%>/orderManage/findAll.do?notEnd=8',
            page: true,
            method:'post',
            height:600,
            limit:200,
            ...
            cols: [[
                ...
                {field:'priorPrice',  title: '<spring:message code='toOrder.priorPrice'/>',align :'center'},
                {field:'miumToPrice',  title: '<spring:message code='toOrder.miumToPrice'/>',align :'center'},
    	 ...
    	 var $ = layui.$, active = {
    			    ...
    			    deleteOrder: function(){
  			    	  var checkStatus = table.checkStatus('queryOrders');
					     ...
					     if(length!=0){
					       layer.confirm("确定预报这<font color='red'>"+length+"</font>条数据？", function(index){ 
									var ids="";
									for(var i=0;i<length;i++){
										if(i==0){ ids += i_ids[i]; }else{ ids += ','+i_ids[i]; }						
									} 
    			    ...
    			    getShip: function(){
					     ...
					     if(length!=0){
					       layer.confirm("确定预报这<font color='red'>"+length+"</font>条数据？", function(index){ 
									...
									var index=layer.load();
									$.post('<%=path%>/orderManage/yubao.do',{ids:ids},
											function(result){
											  var date=eval('('+result+')');
										  if(date.result!='0'){
											  layer.msg(date.result,{icon:2});
										}else{
											  layer.msg('预报成功',{icon:1});
									        		document.location.reload();//因为加了遮罩，添加成功不能刷新页面，于是手动刷新			
									          i_ids = new Array(); 
		...
		<div class="container">
			<ul class="myorder-dropdown ">
				...
				managementdraft managementshipped managementpicked_up managementdelivered active">
					<a href="<%=path%>/queryOrder/to_order.html"><spring:message
							code="orderManager" /></a>
				</li>
	...
	<div class="xoms-thirdnavbar">
		<div class="container order-management-thirdnav">
			<ul class="nav nav-pills" style="margin-top: 30px;">
				<li class="draft"><a href="<%=path%>/orderManage/draft.html">
						<span><i></i></span> <span><spring:message
								code="toOrder.Draft" />（${countVo.to_draft}）<span
							class="nav-order-count"></span></span>
				</a></li>

				<li class="info_received active"><a
						...
						<div class="pull-left xoms-btn-container operate-btn-container">
							<a class="btn layui-btn layui-btn-normal layui-btn-sm export"
								...
								class="glyphicon glyphicon-arrow-down export"></span> <spring:message
									code="toOrder.ExportExcel" /></a> <a class="btn layui-btn layui-btn-normal layui-btn-sm " data-type="getShip">
									<span class="glyphicon glyphicon-ok"></span>
							<spring:message code="toOrder.prediction" /></a>
						...
						<div class="pull-right xoms-keyword-div received-search-container">
                            ...
                            <div class="form-group other-value-group">
								...
								<input type="hidden" name="orderOutCustom.notEnd"
									class="form-control" value="8"> <input type="hidden"
