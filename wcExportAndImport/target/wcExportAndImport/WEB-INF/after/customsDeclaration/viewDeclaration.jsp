...
<script type="text/javascript">
    ...
    layui.use(['form', 'layedit', 'laydate', 'table', 'element','jquery'], function(){
    	 ...
    	 var numberBT= Number($('#number').val());//提单件数
    	 ...
    	 var transactionType= Number($('#transactionType').val());//成交方式
    	 var supervisionType= Number($('#supervisionType').val());//监管方式
    	 var ippc = Number($('input[name="ippc"]:checked').val());//有无标识
    	...
    	var estimatedArrivalTimeValue
    	 laydate.render({
    		 elem: '#estimatedArrivalTime' //指定元素 
    			 ,done: function(value, date){
    		            ...
    		            var estimatedArrivalTimeValue = value;
    		          //时间变动处理
		         		  ...
		         		  if (data.status == 200){
								layer.msg(data.msg,{
									...
									time:500,
										...
										window.location.reload();
										 <%-- location.href = '<%=path%>/page/pageJump.do?path=after/customsDeclaration/customsDeclaration'; --%>
					      				});
			                   }
			                   else{
			                	   layer.msg(data.msg,{
    	 ...
    	 var btDataQJa;//定义表体全局
		     ...
		     /* var ids = ${aArrivalPlan.id}
		     var importPort=$("#importPort").val();
		   	 var estimatedArrivalTime= getUnixTime($("#estimatedArrivalTime").val());
		   	 var mainWaybillNumber=$("#mainWaybillNumber").val();//主运单号
		   	 var fenWaybillNumber=$("#fenWaybillNumber").val();//分运单号
		   	 var overseasShipper=$("#overseasShipper").val();//境外发货人
		   	 var overseasConsignee=$("#overseasConsignee").val();//境外收货人
		   	 var unit=$("#unit").val();//消费使用单位
		   	 var number= Number($('#number').val());//提单件数
		   	 var packageDimensions=$("#packageDimensions").val();//包装尺寸
		   	 var weight=$("#weight").val();//提单重量
		   	 var freight=$("#freight").val();//运费
		   	 var premium=$("#premium").val();//保费
		    ...
		    $(function(){
		    	...
		    	$("select").change(function(){
		    	    ...
		    	    var transactionTypeValue = Number($("#transactionType").val());
				...
				<div class="col-lg-3 saveInput">
					...
					<div class="col-lg-7 valueName">
						...
						<select class="form-control category" name="transactionType"
							id="transactionType">
							<option value="" >请选择</option>
							<%-- <option value="0" <c:if test="${aArrivalPlan.transactionType=='0'}">selected</c:if>>方式三</option> --%>
