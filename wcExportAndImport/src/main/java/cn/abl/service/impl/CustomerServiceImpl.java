	...
	public E3Result updateCustomer(CCustomerInformation cCustomerInformation, String userID) {
		if (cCustomerInformation.getId() != null) { //修改
			...
			if (list.size() > 0 && null == list) {
				...
				user.setStatus(1);
				user.setEmail(cCustomerInformation.getEmail());
				String md5PW = DigestUtils.md5DigestAsHex(cCustomerInformation.getPassWord().getBytes());
				...
				UUser uUser = UUserMapper.selectByPrimaryKey(Integer.valueOf(userID)); // 当前登录用户
				String dataPrivilege = uUser.getDataPrivilege(); // 当前登录用户权限
				uUser.setDataPrivilege(dataPrivilege + "," + String.valueOf(user.getId()));
