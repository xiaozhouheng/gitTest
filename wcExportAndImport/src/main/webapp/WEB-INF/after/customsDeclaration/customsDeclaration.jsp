...
$(function(){
	var options = {
			//初始显示所有地址
			url : '<%=path%>/customsDeclaration/importDeclaration.do',
			//进入修改页面的地址
			editUrl : '<%=path%>/customsDeclaration/editUi.do',
			//进入添加页面的地址
			addUrl :'<%=path%>/customsDeclaration/editUi.do',
			//删除地址     
			removeUrl : '<%=path%>/customsDeclaration/delete.do',
			//进入派单页面地址  
			<%-- orderUrl:'<%=path%>/page/updateJump.do?path=after/customsDeclaration/sendOrder', --%>
			...
			cols: [[ //表头
                   ...
                   ,{field : 'param3', title: '权限派单',align: 'center',templet :tabelSelect}
                   ,{field : 'param3', title: '收款',align: 'center',templet :tabelReceivables}
...
function getSendOrder(id){
	var id = id;
    var urls = "<%=path%>/page/updateJump.do?path=after/customsDeclaration/sendOrder";
    var url = urls+"&id="+id; 
	layer.open({
    	  title: '上传文件',
    	  type: 2,
...
function tabelReceivables(data){
	var  id = data.id;
	var btn = "<a class='layui-btn layui-btn-xs' lay-event='update' lay-filter = 'paiDan' onclick='getReceivables("+id+")'>收款</a>";
	return btn;
}
//进入收款页面地址
function getReceivables(id){
	var id = id;
    var urls = "<%=path%>/page/updateJump.do?path=after/customsDeclaration/ImportReceivables";
    var url = urls+"&id="+id; 
	layer.open({
    	  title: '收款页面',
    	  type: 2,
