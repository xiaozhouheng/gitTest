...
<form class="layui-form">
  <div class="layui-form-item form-inline">
    ...
    <div class="layui-input-inline">
      <input type="text" name="" placeholder="请输入" id="getValue" autocomplete="off" class="layui-input">
    </div>
...
layui.use(['form', 'layedit', 'laydate', 'table', 'layer','form'], function () {
    ...
    table.render({
         url: "<%=path%>/country/getAllCountry.do"
        ,height:550
        ,elem: '#layuiTable'
        , limit: '10'/* 一页显示多少条 */
...
function addUI(id) {
	layer.open({
		...
		content:["<%=path%>/country/editUi.do", "no"]
	    });
...
function editUI() {
	...
	layer.open({
		...
		shade: 0.8,//遮罩透明度
		skin: 'layui-layer-rim',//加边框
		content:["<%=path%>/country/editUi.do?id="+id, "no"],
	    })}
...
function editDel(){
	...
	$.ajax({
	      url:"<%=path%>/country/deleteByID.do?id="+id,
	      async: false,
	      type:"POST",
	      dataType: "text",
